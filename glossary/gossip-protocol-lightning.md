...
Gossip volume grew large enough that newer optimizations (gossip sync v2, range queries, compact filters) became necessary, alongside [periodic pruning](/glossary/lightning-gossip-pruning) of stale channel announcements that haven't been refreshed in weeks. A modern Lightning node downloads tens of megabytes of gossip data when fully syncing. Once synced, ongoing gossip is a few KB/sec at most.
