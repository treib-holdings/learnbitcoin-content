...
- **Bitcoin Knots vs Core** apply different relay policies. Knots is more restrictive about inscription-style transactions and will not relay some that Core will.
...
Fee rate is the fee divided by the transaction's size, expressed in satoshis per virtual byte (**sat/vB**). A 250-byte transaction with a 2,500 sat fee has the same fee rate (10 sat/vB) as a 1,000-byte transaction with a 10,000 sat fee.
...
Here is the current state of the mempool, ordered by fee rate:

<MempoolHistogram client:visible />

Each bucket shows how much pending fee revenue sits at each rate level. The buckets at the top of the histogram are the ones that will be mined first.
