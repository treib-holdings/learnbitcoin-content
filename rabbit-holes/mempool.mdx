...
<figure>
  ...
  ></video>
  <figcaption>Broadcast, propagation, fee-rate sorting, mining, eviction, rebroadcast — the full mempool lifecycle in under a minute.</figcaption>
</figure>
...
- **Evicted.** The mempool has size limits (default 300 MB in Bitcoin Core). When the mempool fills up, transactions with the lowest fee rates get dropped. A dropped transaction can be rebroadcast later, but most wallets do not automate this.
