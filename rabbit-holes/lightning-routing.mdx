...
<figure>
  ...
  ></video>
  <figcaption>Lightning is a graph. One open channel reaches the whole network through routed multi-hop payments.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>One channel is one edge in the Lightning graph: opened on Bitcoin, used off-chain, closed back to Bitcoin if ever. Each end has other channels to the wider network. That is what makes routing possible.</figcaption>
</figure>
...
Capacity is symmetric (the channel has 0.1 BTC total). Balance is asymmetric (it shifts as payments flow). This asymmetry is the source of nearly every Lightning routing failure. The closer to a [balanced channel](/glossary/balanced-channel-lightning) - capacity split roughly 50/50 between both sides - the more reliable that channel is for routing in either direction.
...
<figure>
  ...
  ></video>
  <figcaption>Alice builds the onion inside out. Each hop peels only its own layer.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>One payment splits into chunks across different paths. Path lengths can vary. All chunks succeed, or all return.</figcaption>
</figure>
...
<figure>
  ...
  <figcaption>Top three rows: the privacy story. Middle hops are blind to the endpoints. Bottom three rows: operational truths every hop needs in order to forward.</figcaption>
</figure>
...
A typical [routing node](/glossary/routing-node) might charge `1,000 msat base + 100 ppm`. Routing 0.01 BTC (1,000,000 sats = 1,000,000,000 msat) costs: 1,000 + (100 x 1,000) = 101,000 msat, or roughly 101 sats total fee.
