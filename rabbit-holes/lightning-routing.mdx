...
<figure>
  <img src="/diagrams/lightning-hop-knowledge.svg" alt="A six-column matrix showing what each position on a Lightning route (Sender Alice, First Hop Bob, Middle Hops Carol and Fred, Last Hop Eve, Receiver Dave) knows about a payment. The top three rows (Previous hop, Next hop, Forwarded amount) are nearly solid orange, because every hop has to know its neighbors and the amount in order to forward. The bottom three rows (Original sender, Final receiver, Position in path) are mostly dashes in the middle, with orange clusters at the endpoints. The visual donut in the bottom half is the privacy story: middle hops are blind to who is at the ends." />
  <figcaption>Top three rows: operational truths every hop needs in order to forward. Bottom three rows: the privacy story. Middle hops are blind to the endpoints.</figcaption>
</figure>

The top three rows are operational. Every hop has to know its neighbors and the amount being forwarded, otherwise it cannot route the HTLC at all.

The bottom three rows are the privacy. The matrix shows two important asymmetries:
